...
class Ref extends Product{
	...
	Ref(){  }
	Ref(int price, String name, int liter, int door) {
		this.price = price;
		this.name = name;
		this.liter = liter;
		this.door = door;
...
class Lcd_Tv extends Tv{
	//이름, 가격, 포인트, 화면, 인치
	Lcd_Tv(){
		super("Lcd_TV",1000,"LCD",1);
	}
}
class Led_Tv extends Tv{
	
	Led_Tv(){
		super("Led_TV",2000,"LED",2);
	}
}
class Oled_Tv extends Tv{
	
	Oled_Tv(){
		super("Olcd_Tv",3000,"OLCD",3);
	}
}
...
class D_Ref extends Ref{
	D_Ref(){
		super(1000,"일반냉장고",160,1); //가격, 이름, 리터, 문
	}
}
class DD_Ref extends Ref{
	...
	DD_Ref(){
		super(2000,"양문형냉장고",500,2); //가격, 이름, 리터, 문
	}
